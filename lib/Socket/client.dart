...
class Traccar {
  ...
  Future<Stream<Device>> positions() async {
    ...
    final posStream =
        await _positionsStream(serverUrl: serverUrl, email: email, password: password);
    if (verbose) {
      ...
      if (dataMap.containsKey("positions")) {
        ...
        for (final posMap in dataMap["positions"]) {
          ...
          if (_devicesMap.containsKey(id)) {
            ...
            device = deviceFromPosition(posMap as Map<String,dynamic>, keepAlive: Duration(minutes: keepAlive));
          //deviceFromPosition(posMap as Map<String, dynamic>, keepAlive: Duration(minutes: keepAlive))
          }
  ...
  Future<void> _getCookie({String protocol = "http"}) async {
    ...
    try {
      ...
      response = await _dio.post<Map<String, dynamic>>(addr,
          queryParameters: <String, dynamic>{"email": email , "password" : password});
  ...
  Future<Stream<dynamic>> _positionsStream(
      {required String serverUrl,required String email,required String password,
